...
export function BackupRestorePanel() {
  ...
  return <section className="panel">
    ...
    <p className="status-warn">Backup v1은 승급 provenance를 보존하지만 개인수련·메모·공유 원본 snapshot은 포함하지 않으며 Membership Credential을 백업하지 않습니다. 전체 백업은 후속 Backup v2에서 지원합니다.</p>
    <button className="action-button" type="button" onClick={download} disabled={busy}>Backup v1 파일 만들기</button>
    <h3>전체 복원</h3>
    <p className="status-warn">복원하면 Backup v1 대상 수련·프로필·승급 데이터가 선택한 내용으로 교체됩니다. 저장된 Membership Credential은 변경하지 않습니다. 현재 삼성당 발급 승급이력을 누락한 이전 백업은 차단됩니다.</p>
